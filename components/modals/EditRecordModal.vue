  ...
  <div class="modal-bg">
    ...
    >
      ...
      <div class="form">
        ...
        <ResultButton
          :previousResult="editingRecord.result"
          @clickWin="isWin"
          @clickLose="isLose"
          class="pt-4 pb-2"
        />
        ...
        <div v-show="isShowInputDetails" class="details mt-15 mb-25 px-4">
          ...
          <Checkbox
            ref="isRepeat"
            :defaultValue="editingRecord.isRepeat"
            label="連戦だった"
          />
          <Checkbox
            ref="isVip"
            :defaultValue="editingRecord.isVip"
            label="VIPマッチ"
          />
        </div>
        <div class="pb-4">
...
export default {
  ...
  methods: {
    select() {
      ...
      console.log(
        "submit",
        this.editingRecord.fighterId,
        this.editingRecord.opponentId
      );
    },
    isWin() {
    ...
    async updateRecord() {
      ...
      if (
        !this.editingRecord.fighterId ||
        !this.editingRecord.opponentId ||
        this.editingRecord.result === null
      ) {
        ...
        if (this.originalResult !== updatingRecord.result) {
          const updateUserDto = {
            results: {
              ...
              wins: updatingRecord.result
                ? this.user.results.wins + 1
                : this.user.results.wins - 1,
              loses: updatingRecord.result
    ...
    deleteRecord() {
      ...
      try {
        ...
        const deletedRecords = this.records.filter(
          (record) => record.docId !== this.editingRecord.docId
        );
        this.$store.commit("setRecords", deletedRecords);
        const updateUserDto = {
          results: {
            matches: this.user.results.matches - 1,
            wins: this.editingRecord.result
              ? this.user.results.wins - 1
              : this.user.results.wins,
            loses: this.editingRecord.result
