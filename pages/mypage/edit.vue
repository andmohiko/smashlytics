      ...
      <div class="form">
        <div class="input">
          ...
          <form class="mb-4 px-4">
            <label class="text-gray-700 flex text-left"
              >アイコンを選択する</label
            >
            ...
            <TextField
              ref="friendCode"
              label="フレンドコード"
              :defaultValue="user.friendCode"
              placeholder="SW-xxxx-xxxx-xxxx"
            />
            <TextField
              ref="smashmateRating"
              label="スマメイト 最高レーティング"
              :defaultValue="user.smashmateRating"
              placeholder="1500"
            />
            <TextField
              ref="mainPlayingTime"
  ...
  methods: {
    ...
    selectIcon(e) {
      ...
      const uploadTask = storageRef
        .child(`images/user/${this.user.userId}/profileImg.png`)
        .put(file);
      uploadTask.on(
        "state_changed",
        (snapshot) => {
