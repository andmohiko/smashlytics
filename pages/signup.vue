    ...
    <div class="signup">
      <div class="form">
        ...
        <form class="mb-4 px-4">
          <TextField ref="userId" :allowEmpty="false" label="ユーザID (*)" placeholder="英数字で！" />
          <TextField ref="username" :allowEmpty="false" label="ユーザ名 (*)" placeholder="ユーザ名は日本語もok🙆" />
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      // if (
      //   !this.user.userId.match(/-/) ||
      //   !this.user.userId.match(/@/) ||
      //   !this.user.userId.match(/./) ||
      //   !this.user.userId.match(/,/)
      // ) {
      //   this.error = 'ユーザIDに使えない記号が混ざっています'
      //   return
      // }
      ...
      if (this.user.userId.length < 4) {
        this.error = 'ユーザIDが短すぎます'
        return
      }
      if (this.user.userId.length > 20) {
        this.error = 'ユーザIDが長すぎます'
        return
      }
      // this.error = ''
      // return
